...
function checkSkip()
	if mainmemory.readbyte(0x7444EC) == 1 and mainmemory.read_u16_be(0x7476F4) == 31 then
		return false
	end
	return true;
end
...
gb_coords = {
	...
	while movement_state < 0x89 do
		...
		if isOnBlacklist == 0 then
			...
			if isRDRAM(playerObject) then
				for i = 1, 3 do
					mainmemory.writefloat(playerObject + 0x78 + (4 * i), gb_coords[i], true);
				end
			end
			notCrashed = 0;
			lag_frames = 0;
			...
			for i = 1, 20 do
				...
				if not emu.islagged() then
					...
					skipped = checkSkip();
					if skipped then
						danceSkips[movement_state] = "Yes";
					else
						danceSkips[movement_state] = "No";
					end
				else
			...
			if notCrashed == 0 or lag_frames > 15 then
				danceSkips[movement_state] = "Crashes the game";
			end
		else
			danceSkips[movement_state] = "Crashes BizHawk";
		end
		--if noclipStates[movement_state] == "Crashes BizHawk" then
			print("["..toHexString(movement_state).."] - ("..control_states[movement_state]..") - "..danceSkips[movement_state]);
		--end
